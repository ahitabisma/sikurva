...
@section('content')
    {{-- Default Profile --}}
    <div class="p-5 my-6 border border-gray-200 rounded-2xl lg:p-6 bg-white">
        <h4 class="text-lg font-semibold text-gray-800 mb-4">
            Email dan No Whatsapp untuk Landing Page
        </h4>

        <form method="POST" action="{{ route('super-admin.dashboard.update-contact') }}">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <x-text-input id="email" name="email" type="text" class="mt-1 block w-full" :value="old('email', $contact['email'] ?? '')"
                        required autofocus autocomplete="email" placeholder="Masukkan Email"/>
                ...
                <div>
                    ...
                    <x-text-input id="no_wa" name="no_wa" type="number" class="mt-1 block w-full" :value="old('no_wa', $contact['no_wa'] ?? '')"
                        required autofocus autocomplete="no_wa" placeholder="Masukkan No Whatsapp"/>
                    <x-input-error class="mt-2" :messages="$errors->get('no_wa')" />
                </div>
            </div>

            <div class="flex items-center gap-4 mt-6">
                <x-primary-button class="min-w-fit w-fit">{{ __('Ubah Data') }}</x-primary-button>

                @if (session('status') === 'contact-updated')
                    <x-alert-success title="Data berhasil diperbarui!" />
                @endif
            </div>
        </form>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-4 md:gap-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 md:p-6">
            ...
            <div class="mt-5 flex items-end justify-between">
                <div>
                    <span class="text-sm text-gray-500">Total User Nakes</span>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 md:p-6">
            ...
            <div class="mt-5 flex items-end justify-between">
                <div>
                    <span class="text-sm text-gray-500">Total User Awam</span>
                    <h4 class="mt-2 text-title-sm font-bold text-gray-800">
                        {{ $count['nonNakes'] }}
                    </h4>
                </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 md:p-6">
            ...
            <div class="mt-5 flex items-end justify-between">
                <div>
                    <span class="text-sm text-gray-500">Total Klinik</span>
                    <h4 class="mt-2 text-title-sm font-bold text-gray-800">
                        {{ $count['klinik'] }}
                    </h4>
                </div>
                                    ...
                                    <td class="px-6 py-3 whitespace-nowrap">
                                        <div class="flex items-center justify-center">
                                            <p class="text-gray-700 text-theme-sm">
                                                {{ $lastActivity->user_name ?? $lastActivity->instansi_name }}
                                            </p>
                                        </div>
                                    </td>
                                    <td class="px-6 py-3 whitespace-nowrap">
