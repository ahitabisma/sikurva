...
@section('content')
    <div class="rounded-2xl border border-gray-200 bg-white">
        <div class="p-5 space-y-6 border-t border-gray-100 sm:p-6">
            <form action="{{ route('super-admin.langganan.paket.update', $paket->id) }}" method="POST"
                x-data="{ statusSelected: false, durationTypeSelected: false }">
                @csrf
                @method('PUT')
                <div class="-mx-2.5 flex flex-wrap gap-y-5">
                    ...
                    <div class="w-full px-2.5 xl:w-1/2">
                        ...
                        <x-text-input id="paket" type="text" name="name" required placeholder="Masukkan Nama Paket"
                            :value="$paket->name" />
                    ...
                    <div class="w-full px-2.5 xl:w-1/2">
                        ...
                        <x-text-input id="point" type="number" min="0" name="point" required
                            placeholder="Masukkan Jumlah Point" :value="$paket->point" />
                    ...
                    <div class="w-full px-2.5 xl:w-1/2">
                        ...
                        <x-text-input id="durasi" type="number" min="1" name="duration" required
                            placeholder="Masukkan Durasi" :value="$paket->duration" />
                        ...
                        <div class="relative z-20 bg-transparent">
                            <select required
                                ...
                                <option value="bulan" class="text-gray-500"
                                    {{ $paket->duration_type == 'bulan' ? 'selected' : '' }}>
                                    Bulan
                                </option>
                                <option value="tahun" class="text-gray-500"
                                    {{ $paket->duration_type == 'tahun' ? 'selected' : '' }}>
                                    Tahun
                                </option>
                            </select>
                    ...
                    <div class="w-full px-2.5 xl:w-1/2">
                        ...
                        <x-text-input id="harga" type="number" min="1" name="price" required
                            placeholder="Masukkan Harga (Rp)" :value="$paket->price" />
                    ...
                    {{-- <pre>{{ implode(', ', json_decode($paket->description)) }}</pre>
                    <pre>@json(json_decode($paket->description))</pre> --}}
                    {{-- @json(json_decode($paket->description)) --}}


                    <!-- Tambahkan debug di PHP -->
                    @php
                        $decoded = json_decode($paket->description, true) ?? [''];
                    @endphp

                    <!-- Pisahkan data ke variabel JavaScript -->
                    <script>
                        window.paketDescription = @json($decoded);
                    </script>

                    <div x-data="{ description: window.paketDescription }" x-init="console.log('Description:', description)" class="w-full px-2.5 xl:w-3/4">
