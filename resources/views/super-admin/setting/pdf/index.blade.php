...
@section('content')
    <div class="space-y-6 rounded-2xl border border-gray-200 bg-white p-5" x-data="pdfSettings">
        <form action="{{ route('super-admin.setting.pdf.update') }}" method="POST" id="pdf-setting-form">
            @csrf
            ...
            <div class="mb-6">
                <div class="space-y-4">
                    <template x-for="(setting, index) in pdfSettings" :key="index">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <div class="mt-1 relative">
                                        <input type="text" :id="'input-' + setting.key"
                                            :name="'pdfSettings[' + index + '][value]'"
                                            ...
                                            :placeholder="'Masukkan ' + getSettingName(setting.key)">
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">
                                        <template x-if="setting.key === 'nama'">
                                            Nama yang akan ditampilkan pada dokumen PDF
                                        </template>
                                        <template x-if="setting.key === 'nama_instansi'">
                                            Nama instansi atau klinik yang akan ditampilkan pada dokumen PDF
                                        </template>
                                        <template x-if="setting.key === 'alamat'">
                                            Alamat lengkap yang akan ditampilkan pada dokumen PDF
                                        </template>
                                    </p>
                                    <template x-if="$store.errors && $store.errors['pdfSettings.' + index + '.value']">
                                        <p class="mt-1 text-xs text-red-500"
                                            x-text="$store.errors['pdfSettings.' + index + '.value'][0]"></p>
                                    </template>
                                </div>
                            </div>

                            <input type="hidden" :name="'pdfSettings[' + index + '][id]'" x-model="setting.id" />
                            <input type="hidden" :name="'pdfSettings[' + index + '][key]'" x-model="setting.key" />
                        </div>
                    </template>

                    <div x-show="pdfSettings.length === 0" class="text-center py-8 bg-gray-50 rounded-lg">
                        <p class="text-gray-500">Setting PDF belum dikonfigurasi.</p>
                    </div>
                </div>
            </div>
        </form>
...
@section('script')
    <script>
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('pdfSettings', () => ({
                pdfSettings: @json($settings) || [],
                ...
                clearSettingValue(index, settingId) {
                    if (settingId > 0) {
                        ...
                        window.location.href =
                            "{{ route('super-admin.setting.pdf.clear-value', ['id' => ':id']) }}"
                            .replace(':id', settingId);
                    } else {
                        // For new settings just clear the value in the form
                        this.pdfSettings[index].value = '';
                    }
                }
            }));
        });
