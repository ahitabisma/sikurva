            ...
            <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white pt-4">
                <div class="flex flex-col gap-5 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                        {{-- <x-tambah-button url="{{ route('patient.create') }}" text="Tambah Pasien" /> --}}
                        {{-- <x-import-button url="{{ route('patient.import') }}" text="Import Pasien" /> --}}
                    </div>

                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                        <form method="GET" action="{{ route('patient.index') }}" class="flex flex-col gap-2 lg:flex-row">
                            <div class="relative">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($patients as $patient)
                                <tr>
                                        ...
                                        <div class="flex items-center justify-center">
                                            <div>
                                                <span class="block font-medium text-gray-700 text-theme-sm">
                                                    {{ $patient->kode_lokal_instansi != null ? $patient->kode_lokal_instansi : $patient->kode_lokal_user ?? null }}|{{ str_pad($patient->kode_lokal, 6, '0', STR_PAD_LEFT) }}
                                                </span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-3 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <a href="{{ route('patient.preview', $patient->id) }}"
                                                class="flex items-center gap-3 hover:scale-105 transition ease-in-out duration-300">
                                                <div
                                                    class="flex items-center justify-center w-10 h-10 rounded-full {{ $patient->jenis_kelamin === 'L' ? 'bg-blue-100' : 'bg-red-100' }}">
                                                    <span
