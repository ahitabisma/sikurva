...
@section('content')
    <div x-data="bannerSettings">
            ...
            <template x-if="notification.type === 'success'">
                <div class="relative flex items-center justify-between gap-3 w-full rounded-md bg-white p-3 shadow-theme-sm overflow-hidden"
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex h-10 w-10 items-center justify-center rounded-lg text-success-600 bg-success-50">
        ...
        <form action="{{ route('super-admin.landing-page.banner.update') }}" method="POST" enctype="multipart/form-data"
            id="banner-form">
            @csrf
            @method('PUT')
                        ...
                        <div class="grid gap-8 md:grid-cols-2">
                            ...
                            <div>
                                <div class="rounded-xl border border-gray-200 bg-white p-5 sm:p-6">
                                    <div>
                                        <label :for="'banner-image-' + index"
                                            class="mb-2 text-sm font-medium text-gray-700">
                                            Banner Background Image
                                        </label>
                                        <div class="mt-1 mb-4">
                                            <div
                                                class="overflow-hidden bg-gray-100 border border-gray-200 rounded-md aspect-video">
                                                <img :src="banner.bg_banner" alt="Banner preview"
                                                    class="object-cover w-full h-full" />
                                            </div>
                                        </div>
                                        <input type="file" :name="'banners[' + index + '][bg_banner_file]'"
                                            :id="'banner-image-' + index" accept="image/*"
                                        ...
                                        <input type="hidden" :name="'banners[' + index + '][id]'" x-model="banner.id" />
                                        <input type="hidden" :name="'banners[' + index + '][bg_banner_path]'"
                                            x-model="banner.bg_banner" />
                                        ...
                                        <template
                                            x-if="$store.errors && $store.errors['banners.' + index + '.bg_banner_file']">
                                            <p class="mt-1 text-sm text-red-500"
                                                x-text="$store.errors['banners.' + index + '.bg_banner_file'][0]"></p>
                                        </template>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <div class="rounded-xl border border-gray-200 bg-white p-5 sm:p-6">
                                    <div class="space-y-4">
                                        <div>
                                            <label :for="'title-' + index" class="text-sm font-medium text-gray-700">
                                                Banner Title
                                            </label>
                                            <input :id="'title-' + index" x-model="banner.title"
                                            ...
                                            <template x-if="$store.errors && $store.errors['banners.' + index + '.title']">
                                                <p class="mt-1 text-sm text-red-500"
                                                    x-text="$store.errors['banners.' + index + '.title'][0]"></p>
                                            </template>
                                        </div>

                                        <div>
                                            <label :for="'subtitle-' + index" class="text-sm font-medium text-gray-700">
                                                Banner Subtitle
                                            </label>
                                            <input :id="'subtitle-' + index" x-model="banner.subtitle"
                                                :name="'banners[' + index + '][subtitle]'"
                                                class="mt-1 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-blue-300 focus:outline-hidden focus:ring-3 focus:ring-blue-500/10"
                                                placeholder="Masukkan Subtitle" />
                                            <template
                                                x-if="$store.errors && $store.errors['banners.' + index + '.subtitle']">
                                                <p class="mt-1 text-sm text-red-500"
                                                    x-text="$store.errors['banners.' + index + '.subtitle'][0]"></p>
                                            </template>
                                        </div>

                                        <!-- Tombol Hapus -->
                                        <button type="button" x-show="banners.length > 1"
                                            @click="removeBanner(index, banner.id)"
                                            class="w-full inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors">
                                            Remove Banner
                                        </button>
                                    </div>
                                </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('bannerSettings', () => ({
                ...
                async removeBanner(index, bannerId) {
                    if (this.banners.length <= 1) {
                        this.notification = {
                            type: 'error',
                            message: 'Tidak bisa menghapus banner karena minimal banner berjumlah 1'
                        };
                    ...
                    if (bannerId) {
                        try {
                            const response = await fetch(
                                `/super-admin/landing-page/banner/destroy/${bannerId}`, {
                                    method: 'DELETE',
                                    headers: {
                            ...
                            if (!response.ok) {
                                this.notification = {
                                    type: 'error',
                                    message: data.message || 'Gagal menghapus banner'
                                };
                                setTimeout(() => this.notification = {
                                    type: null,
                                    message: null
                        ...
                        // Remove from frontend if no ID (not yet saved to database)
                        this.banners.splice(index, 1);
                    }
                },
                handleImageUpload(index, event) {
                    ...
                    if (file) {
                        ...
                        reader.onload = (e) => {
                            this.banners[index].bg_banner = e.target.result;
