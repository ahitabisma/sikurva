    ...
    <div class="rounded-2xl border border-gray-200 bg-white mb-6">
        <div class="p-5 space-y-6 border-t border-gray-100 sm:p-6">
            <div class="flex justify-between items-center">
                <div>
                    <a href="{{ route('patient.antro.export-template') }}"
            ...
            <form action="{{ route('patient.antro.import-store', ['patientId' => $patient->id]) }}" method="POST"
                enctype="multipart/form-data" id="import-antro-form" x-data="{
                    'importAntroPoint': {{ $pointSettings->where('name', 'IMPORT-ANTRO')->value('points') ?? 0 }},

                    openConfirmationImportModal() {
                        const customMessage = `
                                                <p>Tindakan import antro akan membutuhkan <strong class='text-blue-500'>${this.importAntroPoint} poin</strong> untuk setiap data pasien yang berhasil diimport.</p>
                        ...
                        window.dispatchEvent(
                            new CustomEvent('open-confirmation-modal', {
                                detail: {
                                    title: 'Import Antro',
                                    points: this.importAntroPoint,
                                    callback: () => document.getElementById('import-antro-form').submit(),
                ...
                <div class="w-full xl:w-full mb-5">
                    ...
                    <p class="mt-1 text-xs text-gray-500">
                        Batasan untuk melakukan import adalah 25 data antro. Jika lebih dari itu, silakan lakukan import secara
                        bertahap.
                    </p>
                ...
                <div class="flex space-x-3">
                    ...
                    <x-cancel-button url="{{ route('patient.preview', ['id' => $patient->id]) }}">Cancel</x-cancel-button>
                </div>
            </form>
        </div>
    </div>
