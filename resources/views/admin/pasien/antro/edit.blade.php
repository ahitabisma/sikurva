...
@section('content')
    <div class="rounded-2xl border border-gray-200 bg-white">
        <div class="p-5 space-y-6 border-t border-gray-100 sm:p-6">
            <form action="{{ route('patient.antro.update', ['id' => $antro->id]) }}" method="POST" x-data="{
                isChecked: false,
                    ...
                    @if (Auth::user()->is_nakes)
                        <div class="w-full px-2.5">
                            ...
                            <x-textarea id="notes" name="notes" rows="3"
                                placeholder="Masukkan catatan tambahan (opsional)">{{ old('notes', $antro->notes) }}</x-textarea>
                            <x-input-error :messages="$errors->get('notes')" class="mt-2" />
                        </div>
                    @endif

                    {{-- <div x-data="{
                            notes: {{ json_encode(old('notes', $antro->parsedNotes ?? [['content' => '']])) }}
                        }" class="w-full px-2.5">
                            <x-input-label for="notes" :value="__('Catatan')" />
                            ...
                            <template x-for="(note, index) in notes" x-bind:key="index">
                                <div class="flex items-center gap-2 mt-2">
                                    <x-textarea x-bind:id="'notes-' + index" name="notes[][content]" rows="2    "
                                        placeholder="Masukkan catatan tambahan" x-model="note.content"
