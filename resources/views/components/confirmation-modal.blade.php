<!-- filepath: d:\laragon\www\closing\ekurva\resources\views\components\confirmation-modal.blade.php -->
...
<div x-data="{
    ...
    open(title, points, callback, customMessage = null, skipConfirmation = false) {
        // Check if Laravel has set the cookie
        const hasCookie = {{ Cookie::has('skip_confirm') ? 'true' : 'false' }};

        // If cookie exists and modal type is specified, skip showing the modal
        if (hasCookie) {
    ...
    handleCheckboxChange() {
        // Don't auto-confirm when simply checking the box
        // This just updates the skipConfirmation state
        this.checkboxToggle = !this.checkboxToggle;
    },

    confirm() {
        // We'll pass the skipConfirmation value to the callback function
        // The callback can then make the request with the skipConfirmation parameter
    ...
    x-on:open-confirmation-modal.window="open($event.detail.title, $event.detail.points, $event.detail.callback, $event.detail.message)"
    ...
    <div x-show="showModal" x-cloak
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 max-w-md mx-auto"
            ...
            <div class="flex flex-col">
                ...
                <!-- Don't show this again checkbox - only show for modal types -->
                <div class="mb-4" @click.stop>
                    <label for="skipConfirmation" class="flex cursor-pointer items-center text-sm select-none"
                        @click.stop>
                        <div class="relative" @click.stop>
                            <input type="checkbox" name="skipConfirmation" id="skipConfirmation" class="sr-only"
                                x-model="skipConfirmation" @click.stop>
